...
static const st7802_lcd_init_cmd_t lcd_init_cmds[] = {
...
esp_err_t hw_lcd_init(esp_lcd_panel_handle_t *panel_handle, esp_lcd_panel_io_handle_t *io_handle, esp_lv_adapter_tear_avoid_mode_t tear_avoid_mode, esp_lv_adapter_rotation_t rotation)
{
    ...
    esp_lcd_dbi_io_config_t dbi_config = ST7802_PANEL_IO_DBI_CONFIG();
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_dbi(s_mipi_dsi_bus, &dbi_config, &s_mipi_dbi_io));

    ESP_LOGI(TAG, "Install LCD driver of st7802");
    esp_lcd_dpi_panel_config_t dpi_config = ST7802_480_480_PANEL_60HZ_DPI_CONFIG(HW_MIPI_DPI_PX_FORMAT);

    dpi_config.num_fbs = esp_lv_adapter_get_required_frame_buffer_count(tear_avoid_mode, rotation);
    st7802_vendor_config_t vendor_config = {
        .init_cmds = lcd_init_cmds,      // Uncomment these line if use custom initialization commands
        .init_cmds_size = sizeof(lcd_init_cmds) / sizeof(st7802_lcd_init_cmd_t),
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_st7802(s_mipi_dbi_io, &panel_config, &s_panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_reset(s_panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_init(s_panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_st7802_set_brightness(s_panel_handle, 50));
