 ...
 *******************************************************************************/
static esp_err_t esp_lcd_touch_cst9220_read_data(esp_lcd_touch_handle_t tp);
static bool esp_lcd_touch_cst9220_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num);
static esp_err_t esp_lcd_touch_cst9220_del(esp_lcd_touch_handle_t tp);
static esp_err_t cst9220_read_reg(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data, size_t len);
static esp_err_t cst9220_write_reg(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data, size_t len);
static esp_err_t cst9220_reset(esp_lcd_touch_handle_t tp);
static esp_err_t cst9220_read_config(esp_lcd_touch_handle_t tp);

/*******************************************************************************
 * Public API functions
 *******************************************************************************/

esp_err_t esp_lcd_touch_new_i2c_cst9220(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    esp_err_t ret = ESP_OK;
    esp_lcd_touch_handle_t cst9220 = NULL;

    ESP_RETURN_ON_FALSE(io && config && out_touch, ESP_ERR_INVALID_ARG, TAG, "Invalid arguments");

    /* Allocate memory for controller */
    cst9220 = heap_caps_calloc(1, sizeof(esp_lcd_touch_t), MALLOC_CAP_DEFAULT);
    ESP_RETURN_ON_FALSE(cst9220, ESP_ERR_NO_MEM, TAG, "No memory for CST9220");

    /* Communication interface */
    cst9220->io = io;

    /* Set callbacks */
    cst9220->read_data = esp_lcd_touch_cst9220_read_data;
    cst9220->get_xy = esp_lcd_touch_cst9220_get_xy;
    cst9220->del = esp_lcd_touch_cst9220_del;

    /* Mutex init */
    cst9220->data.lock.owner = portMUX_FREE_VAL;

    /* Save config */
    memcpy(&cst9220->config, config, sizeof(esp_lcd_touch_config_t));

    if (cst9220->config.int_gpio_num != GPIO_NUM_NC) {
        const gpio_config_t int_gpio_config = {
            .mode = GPIO_MODE_INPUT,
            .intr_type = (cst9220->config.levels.interrupt ? GPIO_INTR_POSEDGE : GPIO_INTR_NEGEDGE),
            .pin_bit_mask = BIT64(cst9220->config.int_gpio_num)
        };
        ESP_GOTO_ON_ERROR(gpio_config(&int_gpio_config), err, TAG, "GPIO intr config failed");

        /* Register interrupt callback */
        if (cst9220->config.interrupt_callback) {
            esp_lcd_touch_register_interrupt_callback(cst9220, cst9220->config.interrupt_callback);
        }
    }

    /* Initialize reset GPIO */
    if (cst9220->config.rst_gpio_num != GPIO_NUM_NC)
    {
        gpio_config_t rst_gpio_cfg = {
            .pin_bit_mask = BIT64(cst9220->config.rst_gpio_num),
    ...
    /* Hardware reset */
    ESP_GOTO_ON_ERROR(cst9220_reset(cst9220), err, TAG, "Reset failed");

    /* Read chip configuration */
    ESP_GOTO_ON_ERROR(cst9220_read_config(cst9220), err, TAG, "Read config failed");
...
err:
    if (cst9220)
    {
        esp_lcd_touch_cst9220_del(cst9220);
    }
    return ret;
}
...
static esp_err_t esp_lcd_touch_cst9220_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_GOTO_ON_ERROR(
        cst9220_read_reg(tp, ESP_LCD_TOUCH_CST9220_DATA_REG, data, sizeof(data)),
        err, TAG, "Read data failed");

    if (data[6] != CST9220_ACK_VALUE) {
        // ESP_LOGE(TAG, "Invalid ACK: 0x%02X vs 0x%02X", data[6], CST9220_ACK_VALUE);
...
static esp_err_t cst9220_read_config(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_RETURN_ON_ERROR(
        cst9220_write_reg(tp, ESP_LCD_TOUCH_CST9220_CMD_MODE_REG, cmd_mode, sizeof(cmd_mode)),
        TAG, "Enter command mode failed");
    vTaskDelay(pdMS_TO_TICKS(10));

    ESP_RETURN_ON_ERROR(
        cst9220_read_reg(tp, ESP_LCD_TOUCH_CST9220_CHECKCODE_REG, data, 4),
        TAG, "Read checkcode failed");
    ESP_LOGI(TAG, "Checkcode: 0x%02X%02X%02X%02X",
           data[0], data[1], data[2], data[3]);

    ESP_RETURN_ON_ERROR(
        cst9220_read_reg(tp, ESP_LCD_TOUCH_CST9220_RESOLUTION_REG, data, 4),
    ...
    ESP_RETURN_ON_ERROR(
        cst9220_read_reg(tp, ESP_LCD_TOUCH_CST9220_PROJECT_ID_REG, data, 4),
